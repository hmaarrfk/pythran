...
namespace types
{
  ...
  namespace details
  {
    ...
    {
      return {init_shape_element<Is>(args, valid_indices<std::tuple_size<pS>::value, Args>{})...};
    }

    template <class pS, class Args>
    pS init_shape(Args const &args)
    {
      return init_shape<pS>(args, utils::make_index_sequence<std::tuple_size<pS>::value>());
    }
  }
